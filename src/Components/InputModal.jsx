        ...
        <div className="modal-box flex flex-col gap-3" onClick={(e)=>e.stopPropagation()}>
          <form onSubmit={handleSubmit}>
            {title.map((title, index) => {
              if(title === "Username" || title === "Universitas" || title === "Kategori" || title === "Kelas" || title === "Sesi" || title === "Individual Mentor"){
                return <label key={index} className="flex items-center gap-2 mt-5">
                  <h1 className="w-[25%]">{title}</h1>
                  <Select
                      className="w-[75%] rounded-md"
                      value={formValues[title]} // Use the corresponding state from formValues
                      onChange={(selectedOption) => handleChange(title, selectedOption)} // Pass the title to handleChange
